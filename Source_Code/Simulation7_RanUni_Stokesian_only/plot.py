...
plt.imshow(field, cmap='coolwarm', extent=[-np.pi, np.pi, -np.pi, np.pi], origin='lower')
plt.colorbar(label='Vorticity Magnitude')
plt.title("No-Flow Condition: $\omega(x, y) = 0$")
plt.xlabel("x")
plt.ylabel("y")
...
def animate_with_trails(
    ...
    trails = [ax.plot([], [], lw=1, color='steelblue', alpha=0.3)[0] for _ in range(N)]
