...
mean_T1 = np.load("mean_speeds(T=1).npy")
mean_T5 = np.load("mean_speeds(T=5).npy")
mean_T3 = np.load("mean_speeds(T=3).npy")
mean_T15 = np.load("mean_speeds(T=15).npy")
# Time step calculation based on known simulation duration
sim_duration = 200  # seconds
n_points = len(mean_T1)
dt = sim_duration / n_points

# Time axes for each simulation
time_T1 = np.arange(len(mean_T1)) * dt
time_T3 = np.arange(len(mean_T3)) * dt
time_T5 = np.arange(len(mean_T5)) * dt

time_T15 = np.arange(len(mean_T15)) * dt
...
plt.plot(time_T1, mean_T1, label=r'Timestep $T = 1$', lw=2)
plt.plot(time_T5, mean_T5, label=r'Timestep $T = 5$', lw=2)
plt.plot(time_T3, mean_T3, label=r'Timestep $T = 3$', lw=2)
plt.plot(time_T15, mean_T15, label=r'Timestep $T = 15$', lw=2)
