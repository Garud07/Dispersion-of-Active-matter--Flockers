...
class particles:
    def __init__(self, N, dist, bases, T, v0, r_int, rho_p, a, rho_f, nu, noise, scale=None, loc=None):
        ...
        self.vi_star = np.ones((self.N, self.dim)) * 0.2

    def initialise_positions(self, scale=None, loc=None):
        # Uniform random distribution across entire domain
        if (rank == 0):
            # Generate positions uniformly in [xmin, xmax] × [ymin, ymax]
            self.positions = np.column_stack([
                np.random.uniform(low=self.coord_boundaries[i][0],
                                  high=self.coord_boundaries[i][1],
                                  size=self.N)
        ...
        # Broadcast positions to all MPI ranks
        self.positions = comm.bcast(self.positions, root=0)
